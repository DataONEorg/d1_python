...
class TestSciMeta(d1_test.d1_test_case.D1TestCase):
  def test_1000(self):
    ...
    expected message"""
    # xml_str = self.load_utf8_to_str('scimeta_isotc211_1.xml')
  ...
  def test_1020(self):
    ...
    xml_str = self.test_files.load_bin('scimeta_dc_1.xml')
  ...
  def test_1050(self):
    ...
    xml_str = self.test_files.load_bin('scimeta_eml_valid.xml')
    format_id = 'eml://ecoinformatics.org/eml-2.1.1'
    scimeta.validate(format_id, xml_str)

  def test_1060(self):
    """SciMeta.validate(): Invalid EML 2.1.1: Unexpected element"""
    xml_str = self.test_files.load_bin('scimeta_eml_invalid_1.xml')
    format_id = 'eml://ecoinformatics.org/eml-2.1.1'
    with pytest.raises(
