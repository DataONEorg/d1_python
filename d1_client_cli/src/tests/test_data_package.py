...
def test_040(self):
  ...
  def test_051(self):
    '''Test 051: Save and Load a package.'''
    now = datetime.datetime.now()
    pkg_pid = now.strftime('test_051_%Y%m%dT%H%MZ') 
    save_pkg = data_package.DataPackage(pkg_pid)
    # Create a package and store it.
    save_pkg.scimeta_add(self.sess, 'knb-lter-gce.294.17')
    save_pkg.scidata_add(self.sess, 'knb-lter-gce.196.27')
    save_pkg.scidata_add(self.sess, 'knb-lter-gce.128.27')
    new_pid = save_pkg.save(self.sess)
    self.assertEqual(pkg_pid, new_pid, 'Couldn\'t save "test_051"')
#    try:
#      load_pkg = data_package.DataPackage(pkg_pid)
#      pkg = load_pkg.load(self.sess)
#      self.assertNotEqual(None, pkg, 'Couldn\'t load package "test_051".')
#      self.assertEqual(pkg_pid, pkg.original_pid, 'Original pid not set.')
#      self.assertNotEqual(None, pkg.sysmeta, 'System Metadata not set.')
...
#                           'No Science Data Object "knb-lter-gce.196.27" found')
#      self.assertFalse(pkg.is_dirty(), 'Package is dirty.')
#      
#    finally:
      try:
