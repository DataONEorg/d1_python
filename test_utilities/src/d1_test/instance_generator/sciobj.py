...
def generate_reproducible_sciobj_with_sysmeta(client, pid=None, option_dict=None):
    """Generate science object bytes and a random, fully populated System
    Metadata object that is always the same for a given PID.
...
def generate_reproducible_sciobj_bytes(pid):
    """Return a bytes object containing a set of bytes that is unique and
    always the same for a given PID.
