...
def sn_client():
  with tempfile.TemporaryDirectory() as tmp_dir_path:
    yield d1_test.slender_node_test_client.SlenderNodeTestClient(sciobj_store_path=tmp_dir_path)


class TestSlenderNodeTestClient(d1_test.d1_test_case.D1TestCase):
  def _generate_sciobj_with_defaults(
      self, client, pid=True, sid=None
  ):
  ...
  def test_1030(self, sn_client):
    ...
    assert d1_common.xml.get_req_val(sysmeta_pyxb.identifier) == \
           d1_common.xml.get_req_val(read_sysmeta_pyxb.identifier)
    assert d1_common.xml.get_opt_val(sysmeta_pyxb, 'seriesId') == \
  ...
  def test_1040(self, sn_client):
    ...
    sn_client.update(old_pid, io.BytesIO(new_sciobj_bytes), new_pid, new_sysmeta_pyxb)
    #
    ...
    assert d1_common.xml.get_opt_val(read_old_sysmeta_pyxb, 'obsoletedBy') == new_pid
  ...
  def test_1050(self, sn_client):
    ...
    with pytest.raises(AssertionError, match='Attempted to create chain with conflicting SIDs'):
      sn_client.update(old_pid, io.BytesIO(new_sciobj_bytes), new_pid, new_sysmeta_pyxb)
