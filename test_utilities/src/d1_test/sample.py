...
def get_sxs_diff(left_obj, right_obj):
    return _get_sxs_diff_str(obj_to_pretty_str(left_obj), obj_to_pretty_str(right_obj))


def gui_sxs_diff(left_obj, right_obj):
    return _gui_diff_str_str(obj_to_pretty_str(left_obj), obj_to_pretty_str(right_obj))
...
def obj_to_pretty_str(o, no_clobber=False, no_wrap=False):
    ...
    def serialize(o):
        ...
        with ignore_exceptions():
            return '\n'.join(o.serialize_to_display(doc_format='nt').splitlines())
...
def _format_file_name(client, filename_postfix_str, extension_str):
    section_list = [get_test_module_name(), filename_postfix_str]
    if client:
        section_list.extend(
            [
