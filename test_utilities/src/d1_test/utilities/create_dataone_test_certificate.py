...
def create_cert(options, args):
    ...
    d1_x509v3_certificate_generator.generate(
        crt_name,
        options.ca_path,
        options.ca_cert_key_path,
        options.ca_key_pw,
        options.public_cert_key_path,
        subject_info,
        options.subject_alt_name,
        dn,
        1 if options.long_term else 0,
    )


def main():
    parser = optparse.OptionParser()
    parser.add_option('--subject-info-path', action='store', type='string')
    parser.add_option(
        '--subject-alt-name', action='store', type='string', default='DNS:dataone.org'
    )
    parser.add_option(
        '--long-term',
        action='store_true',
    ...
    parser.add_option('--ca-path', action='store', type='string', default='ca.crt')
    parser.add_option(
        '--ca-key-path',
        dest='ca_cert_key_path',
        action='store',
        type='string',
        default='ca.key',
    )
    parser.add_option('--ca-key-pw', action='store', type='string', default='my_ca_pw')

    parser.add_option(
        '--public-key-path',
        dest='public_cert_key_path',
        action='store',
        type='string',
        default='pubkey.pem',
    )
