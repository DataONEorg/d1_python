...
class SlenderNodeTestClient:
  ...
  """
  def __init__(self, sciobj_store_path=DEFAULT_SCIOBJ_STORE_PATH, keep_existing=False,
               *args, **kwargs):
  ...
  def update(self, old_pid, sciobj_file, new_pid, new_sysmeta_pyxb, *args, **kwargs):
    ...
    if old_sid is not None:
      assert new_sid is None or new_sid == old_sid, 'Attempted to create chain with conflicting SIDs'
  ...
  def _delete_existing(self):
    ...
    for dir_entry in list(os.scandir(self.sciobj_store_path)):
      if dir_entry.is_file():
        ...
        if dir_entry.name == SID_MAP_FILENAME or ext_str in (SCIOBJ_EXT_STR, SYSMETA_EXT_STR):
