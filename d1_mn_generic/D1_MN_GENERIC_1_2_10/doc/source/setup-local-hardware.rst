...
:term:`GMN` is installed on a physical or virtual machine. Network connectivity is
...
GMN can be used in a mode where it provides a DataONE interface to data that is
already available on the web. When used in this way, GMN must also have access
to the web site which holds the data.
...
To give an indication of the hardware that may be required for hosting GMN, some
