...
class TestCleanUp(unittest2.TestCase):
  ...
  def testCleanUpWithErrors(self):
    ...
    (test1, (Type1, instance1, _)), (test2, (Type2, instance2, _)) = reversed(
      MockResult.errors
    )
  ...
  def testCleanupInRun(self):
    ...
    self.assertEqual(
      ordering, [
        'setUp', 'test', 'tearDown', 'cleanup2', 'cleanup1', 'success'
      ]
    )
...
class Test_TestCase(unittest2.TestCase, EqualityMixin, HashingMixin):
  ...
  ne_pairs = [
    (Test.Foo('test1'), Test.Foo('runTest')), (Test.Foo('test1'), Test.Bar('test1')),
  ...
  def test_run_call_order__error_in_setUp_default_result(self):
    ...
    Foo(events).run()
    expected = [
      'startTestRun', 'startTest', 'setUp', 'addError', 'stopTest', 'stopTestRun'
    ]
    self.assertEqual(events, expected)
  ...
  def test_run_call_order__error_in_test_default_result(self):
    ...
    expected = [
      'startTestRun', 'startTest', 'setUp', 'test', 'addError', 'tearDown', 'stopTest',
      'stopTestRun'
    ]
    Foo(events).run()
    self.assertEqual(events, expected)
  ...
  def test_run_call_order__failure_in_test_default_result(self):
    ...
    expected = [
      'startTestRun', 'startTest', 'setUp', 'test', 'addFailure', 'tearDown', 'stopTest',
      'stopTestRun'
    ]
    events = []
    Foo(events).run()
  ...
  def test_run_call_order__error_in_tearDown_default_result(self):
    ...
    Foo(events).run()
    expected = [
      'startTestRun', 'startTest', 'setUp', 'test', 'tearDown', 'addError', 'stopTest',
      'stopTestRun'
    ]
    self.assertEqual(events, expected)
  ...
  def test_run__uses_defaultTestResult(self):
    ...
    expected = [
      'startTestRun', 'startTest', 'test', 'addSuccess', 'stopTest', 'stopTestRun'
    ]
    self.assertEqual(events, expected)
  ...
  def testAssertDictContainsSubset(self):
    ...
    self.assertRaises(
      unittest2.TestCase.failureException, self.assertDictContainsSubset, {'a': 1,
                                                                           'c': 1},
      {'a': 1}, '.*Missing:.*'
    )
    ...
    self.assertRaises(
      self.failureException, self.assertDictContainsSubset, {1: "one"}, {}
    )
  ...
  def testAssertEqual(self):
    ...
    unequal_pairs = [
      ((), []), ({}, set()), (set([4, 1]), frozenset([4, 2])),
      (frozenset([4, 5]), set([2, 3])), (set([3, 4]), set([5, 4]))
    ]
  ...
  def testEquality(self):
    ...
    self.assertRaises(
      unittest2.TestCase.failureException, self.assertListEqual, tuple(a), tuple(b)
    )
    self.assertRaises(
      unittest2.TestCase.failureException, self.assertSequenceEqual, a, tuple(b)
    )
    ...
    d['x'] = 0
    self.assertRaises(
      unittest2.TestCase.failureException, self.assertDictEqual, c, d, 'These are unequal'
    )
  ...
  def testAssertItemsEqual(self):
    ...
    self.assertRaises(
      self.failureException, self.assertItemsEqual, [10, 11, 10], [10, 11]
    )
    ...
    self.assertRaises(
      self.failureException, self.assertItemsEqual, [2j, None], [None, 3j]
    )
...
"""
    ...
    for type_changer in (lambda x: x, lambda x: x.decode('utf8')):
      try:
        self.assertMultiLineEqual(
          type_changer(sample_text), type_changer(revised_sample_text)
        )
  ...
  def testAssertRegexpMatches(self):
    ...
    self.assertRaises(self.failureException, self.assertRegexpMatches, 'saaas', r'aaaa')
  ...
  def testAssertNotRaisesRegexp(self):
    self.assertRaisesRegexp(
      self.failureException, '^Exception not raised$', self.assertRaisesRegexp, Exception,
      re.compile('x'), lambda: None
    )
    self.assertRaisesRegexp(
      self.failureException, '^Exception not raised$', self.assertRaisesRegexp, Exception,
      'x', lambda: None
  ...
  def testAssertRaisesRegexpMismatch(self):
    ...
    self.assertRaisesRegexp(
      self.failureException, r'"\^Expected\$" does not match "Unexpected"',
      self.assertRaisesRegexp, Exception, '^Expected$', Stub
    )
    self.assertRaisesRegexp(
      self.failureException, r'"\^Expected\$" does not match "Unexpected"',
      self.assertRaisesRegexp, Exception, u'^Expected$', Stub
    )
    self.assertRaisesRegexp(
      self.failureException, r'"\^Expected\$" does not match "Unexpected"',
      self.assertRaisesRegexp, Exception, re.compile('^Expected$'), Stub
    )
