...
from unittest2.util import (safe_repr, safe_str, strclass, unorderable_list_difference)
  ...
  def __exit__(self, exc_type, exc_value, tb):
    ...
    if not expected_regexp.search(str(exc_value)):
      raise self.failureException(
        '"%s" does not match "%s"' % (expected_regexp.pattern, str(exc_value))
      )
    return True
...
class TestCase(unittest.TestCase):
  ...
  def _addSkip(self, result, reason):
    ...
    if addSkip is not None:
      ...
      warnings.warn(
        "Use of a TestResult without an addSkip method is deprecated", DeprecationWarning,
        2
      )
  ...
  def assertNotRegexpMatches(self, text, unexpected_regexp, msg=None):
    ...
    if match:
      ...
      msg = '%s: %r matches %r in %r' % (
        msg, text[match.start():match.end(
        )], unexpected_regexp.pattern, text
      )
      raise self.failureException(msg)
  ...
  def __hash__(self):
    return hash(
      (
        type(
          self
        ), self._setUpFunc, self._tearDownFunc, self._testFunc, self._description
      )
    )
