...
class UserGroupConfig(Base):
  ...
  id = Column(Integer, nullable=False, primary_key=True)
  mechanize_global_configs_id = Column(
    Integer, ForeignKey(
      'mechanize_global_configs.id'
    ), nullable=False
  )
...
class ResultRow(Base):
  ...
  __table_args__ = (UniqueConstraint('run_id', 'trans_count', name='uix_1'))

  id = Column(Integer, nullable=False, primary_key=True)
  mechanize_global_configs_id = Column(
    Integer, ForeignKey('mechanize_global_configs.id'),
    nullable=False
  )
  ...
  def __repr__(self):
    return "<ResultRow('%s','%s','%i','%.3f','%i','%s','%.3f','%s','%s')>" % (
      self.project_name, self.run_id, self.trans_count, self.elapsed, self.epoch,
      self.user_group_name, self.scriptrun_time, self.error, self.custom_timers
    )
...
class TimerRow(Base):
  ...
  id = Column(Integer, nullable=False, primary_key=True)
  mechanize_results_id = Column(
    Integer, ForeignKey('mechanize_results.id'),
    nullable=False
  )
  ...
  result_rows = relation(
    "ResultRow", primaryjoin="TimerRow.mechanize_results_id==ResultRow.id"
  )


def load_results_database(
  project_name, run_localtime, results_dir, results_database, run_time, rampup,
  ...
  run_id = datetime(
    run_localtime.tm_year, run_localtime.tm_mon, run_localtime.tm_mday,
  ...
  for i, ug_config in enumerate(user_group_configs):
    user_group_config = UserGroupConfig(
      ug_config.name, ug_config.num_threads, ug_config.script_file
    )
  ...
  for line in fileinput.input([results_file]):
    ...
    if match:
      result_row = ResultRow(
        project_name, run_id, match.group(1), match.group(2), match.group(3),
