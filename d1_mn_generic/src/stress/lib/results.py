...
def output_results(
  ...
  graph.resp_graph_raw(
    trans_timer_points, 'All_Transactions_response_times.png', results_dir
  )
  ...
  report.write_line('</table>')
  graph.resp_graph(
    avg_resptime_points, percentile_80_resptime_points, percentile_90_resptime_points,
  ...
  for timer_name in sorted(results.uniq_timer_names):
    ...
    for resp_stats in results.resp_stats_list:
      try:
        ...
        pass
    graph.resp_graph_raw(
      custom_timer_points, timer_name + '_response_times.png', results_dir
    )
    ...
    report.write_line(
      '<tr><td>%i</td><td>%.3f</td><td>%.3f</td><td>%.3f</td><td>%.3f</td><td>%.3f</td><td>%.3f</td><td>%.3f</td></tr>'
      % (
        len(custom_timer_vals), min(custom_timer_vals), average(custom_timer_vals),
        percentile(custom_timer_vals, 80), percentile(custom_timer_vals, 90),
...
class Results(object):
  def __init__(self, results_file_name, run_time):
    ...
    self.finish_datetime = time.strftime(
      '%Y-%m-%d %H:%M:%S', time.localtime(
        self.epoch_finish
      )
    )
  ...
  def __parse_file(self):
    ...
    for line in f:
      ...
      r = ResponseStats(
        request_num, elapsed_time, epoch_secs, user_group_name, trans_time, error,
        custom_timers
      )
...
class ResponseStats(object):
  def __init__(
    self, request_num, elapsed_time, epoch_secs, user_group_name, trans_time, error,
    custom_timers
  ):
