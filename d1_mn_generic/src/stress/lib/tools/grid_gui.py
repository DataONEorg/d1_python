...
NODES = ['192.168.1.2:9001', '192.168.1.3:9001', ]
...
class Application:
  def __init__(self, root, hosts):
    ...
    Tkinter.Button(
      self.root,
      text='Get Project Names',
      command=self.get_project_names,
      width=15
    ).place(
      x=5, y=65
    )
    Tkinter.Button(self.root,
                   text='Get Configs',
                   command=self.get_configs,
                   width=15).place(
                     x=5, y=95
                   )
    Tkinter.Button(
      self.root, text='Update Configs',
    ...
    for host, port in self.hosts:
      ...
      try:
        ...
        self.text_box.insert(
          Tkinter.END, 'can not make connection to: %s:%s\n' % (
            host, port
          )
        )
  ...
  def update_configs(self):
    ...
    for host, port in self.hosts:
      ...
      try:
        ...
        self.text_box.insert(
          Tkinter.END, '%s:%s config updated:\n%s\n\n' % (
            host, port, status
          )
        )
      except socket.error:
