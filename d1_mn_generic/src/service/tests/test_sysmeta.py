...
class TestSysMeta(django.test.TestCase):
  ...
  def _create_sci_obj_base(self):
    ...
    sciobj_model.submitter = mn.models.subject('test_submitter')
    sciobj_model.rights_holder = mn.models.subject('rightsHolder0')
    sciobj_model.origin_member_node = mn.models.node('test_origin_mn')
    sciobj_model.authoritative_member_node = mn.models.node('test_auth_mn')
  ...
  def test_100(self):
    ...
    mn.sysmeta._access_policy_pyxb_to_model(sciobj_model, orig_sysmeta_pyxb)
    gen_access_policy_pyxb = mn.sysmeta._access_policy_model_to_pyxb(sciobj_model)
  ...
  def test_200(self):
    orig_sysmeta_pyxb = util.read_test_xml('sysmeta_v2_0_sample.xml')
    sciobj_model = self._create_sci_obj_base()
    mn.sysmeta._replication_policy_pyxb_to_model(sciobj_model, orig_sysmeta_pyxb)
    gen_replication_policy_pyxb = mn.sysmeta._replication_policy_model_to_pyxb(
      sciobj_model
    )
    ...
    for blocked_member_node in gen_replication_policy_pyxb.blockedMemberNode:
      self.assertIn(blocked_member_node.value(), ['blockedMemberNode0', 'blockedMemberNode1'])
    ...
    for preferred_member_node in gen_replication_policy_pyxb.preferredMemberNode:
      self.assertIn(preferred_member_node.value(), ['preferredMemberNode0', 'preferredMemberNode1'])
