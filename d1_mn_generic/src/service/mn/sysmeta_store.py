...
def read_sysmeta_from_store(pid, serial_version):
  ...
  file_in_path = util.store_path(settings.SYSMETA_STORE_PATH, pid, serial_version)
  # Can't use "with".
  f = open(file_in_path, 'rb')
  return util.fixed_chunk_size_iterator(f)


def delete_sysmeta_from_store(pid, serial_version):
  sysmeta_path = util.store_path(settings.SYSMETA_STORE_PATH, pid, serial_version)
  try:
