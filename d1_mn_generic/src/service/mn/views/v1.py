...
import service.mn.db_filter as db_filter
import service.mn.event_log as event_log
import service.mn.models as models
import service.mn.node as node
# import service.mn.psycopg_adapter as psycopg_adapter
import service.mn.restrict_to_verb as restrict_to_verb
import service.mn.sysmeta_store as sysmeta_store
import service.mn.sysmeta_validate as sysmeta_validate
...
def get_monitor_ping(request):
    '''MNCore.ping() → Boolean
  '''
    response = view_shared.http_response_with_boolean_true_type()
    view_shared.add_http_date_to_response_header(response, datetime.datetime.utcnow())
    return response
...
# infrastructure.
@restrict_to_verb.get
@auth.assert_get_log_records_access
def get_log(request):
    '''MNCore.getLogRecords(session[, fromDate][, toDate][, pidFilter][, event]
  [, start=0][, count=1000]) → Log
  '''
    query = models.EventLog.objects.order_by('-date_logged').select_related()
    if not auth.is_trusted_subject(request):
        query = db_filter.add_access_policy_filter(query, request,
                                                   'object__id')
    query = db_filter.add_datetime_filter(query, request, 'date_logged',
                                          'fromDate', 'gte')
    query = db_filter.add_datetime_filter(query, request, 'date_logged',
                                          'toDate', 'lt')
    query = db_filter.add_string_filter(query, request, 'event__event',
                                        'event')
    query = db_filter.add_string_begins_with_filter(query, request,
                                                    'object__pid',
                                                    'pidFilter')
...
def _get_object_byte_stream_local(pid):
    file_in_path = util.store_path(service.settings.OBJECT_STORE_PATH, pid)
    # Can't use "with".
...
def get_meta_pid(request, pid):
  ...
  '''
    ...
    event_log.read(pid, request)
    sciobj = models.ScienceObject.objects.get(pid=pid)
    return HttpResponse(sysmeta_store.read_sysmeta_from_store(pid,
                                                              sciobj.serial_version),
...
@restrict_to_verb.get
@auth.assert_list_objects_access
def get_object(request):
  ...
  '''
    ...
    # for them.
    query = models.ScienceObject.objects.order_by('mtime').select_related()
    if not auth.is_trusted_subject(request):
        query = db_filter.add_access_policy_filter(query, request, 'id')
    query = db_filter.add_datetime_filter(query, request, 'mtime', 'fromDate',
                                          'gte')
    query = db_filter.add_datetime_filter(query, request, 'mtime', 'toDate',
                                          'lt')
    query = db_filter.add_string_filter(query, request, 'format__format_id',
                                        'formatId')
    db_filter.add_bool_filter(query, request, 'replica', 'replicaStatus')
    query_unsliced = query
    query, start, count = db_filter.add_slice_filter(query, request)
    return {'query': query, 'start': start, 'count': count,
...
def post_error(request):
  ...
  '''
    view_asserts.post_has_mime_parts(request, (('file', 'message'),))
    view_asserts.xml_document_not_too_large(request.FILES['message'])
...
def get_replica_pid(request, pid):
  ...
  '''
    ...
    event_log.replicate(pid, request)
    return response
...
def _assert_node_is_authorized(request, pid):
    try:
        ...
        raise d1_common.types.exceptions.ServiceFailure(0,
                                                        'getaddrinfo() failed for "{0}"'.format(
                                                            service.settings.DATAONE_ROOT))
...
def get_is_authorized_pid(request, pid):
  ...
  '''
    ...
    auth.assert_allowed(request, level, pid)
    return view_shared.http_response_with_boolean_true_type()
...
@auth.decorator_assert_create_update_delete_permission
def object_post(request):
  ...
  '''
    ...
    view_asserts.obsoleted_by_not_specified(sysmeta)
    view_asserts.obsoletes_not_specified(sysmeta)
    new_pid = request.POST['pid']
    _create(request, new_pid, sysmeta)
    return view_shared.http_response_with_identifier_type(new_pid)


@auth.assert_write_permission  # OLD object
def put_object_pid(request, old_pid):
  ...
  '''
    ...
    sysmeta = view_shared.deserialize_system_metadata(
        sysmeta_xml.encode('utf-8'))
    view_asserts.obsoletes_matches_pid_if_specified(sysmeta, old_pid)
...
def _create(request, pid, sysmeta):
    view_asserts.xml_document_not_too_large(request.FILES['sysmeta'])
    view_asserts.obsoleted_by_not_specified(sysmeta)
    sysmeta_validate.validate_sysmeta_against_uploaded(request, pid, sysmeta)
    sysmeta_validate.update_sysmeta_with_mn_values(request, sysmeta)
...
def _set_obsoleted_by(obsoleted_pid, obsoleted_by_pid):
    sciobj = models.ScienceObject.objects.get(pid=obsoleted_pid)
...
def post_generate_identifier(request):
  ...
  '''
    view_asserts.post_has_mime_parts(request, (('field', 'scheme'),))
...
def delete_object_pid(request, pid):
  ...
  '''
    ...
    sysmeta_store.delete_sysmeta_from_store(pid, sciobj.serial_version)
...
def _set_archived_flag(pid):
    sciobj = models.ScienceObject.objects.get(pid=pid)
    with sysmeta_store.sysmeta(pid, sciobj.serial_version) as m:
        m.archived = True
        sciobj.serial_version = m.serialVersion
...
def post_replicate(request):
  ...
  '''
    ...
    view_asserts.pid_does_not_exist(sysmeta.identifier.value())
    view_asserts.pid_has_not_been_accepted_for_replication(sysmeta.identifier.value())
    ...
    if service.settings.REPLICATION_MAXOBJECTSIZE != -1:
        if sysmeta.size > service.settings.REPLICATION_MAXOBJECTSIZE:
            raise d1_common.types.exceptions.InvalidRequest(0,
                                                            ...
                                                            .format(service.settings.REPLICATION_MAXOBJECTSIZE,
                                                                    sysmeta.size))
...
def _get_total_size_of_replicated_objects():
    ...
    if total is not None:
        return total
    total = models.ScienceObject.objects.filter(replica=True) \
...
def _create_replication_work_item(request, sysmeta):
    replication_item = models.ReplicationQueue()
