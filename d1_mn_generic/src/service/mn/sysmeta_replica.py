...
def is_obsolescence_chain_placeholder(pid):
  ...
  return mn.models.ReplicaObsolescenceChainReference.objects.filter(
    pid__did=pid).exists()
...
def get_total_size_of_completed_replicas():
  ...
  """
  return mn.models.LocalReplica.objects.aggregate(
...
def add_to_replication_queue(source_node_urn, sysmeta):
  ...
  """
  replica_info_model = mn.models.replica_info(
    status_str='queued',
    source_node_urn=source_node_urn,
  )
  local_replica_model = mn.models.local_replica(
    pid=sysmeta.identifier.value(),
    replica_info_model=replica_info_model,
  )
  mn.models.replication_queue(
...
def replica_model_to_pyxb(sciobj_model):
  replica_pyxb_list = []
  for replica_model in mn.models.RemoteReplica.objects.filter(sciobj=sciobj_model):
