...
class DataONEClientWrapper(d1_client.d1client.DataONEClient):
  ...
  def set_replication_status(self, status, node_ref, pid):
    ...
    url = urlparse.urljoin(
      self.getSetReplicationStatusUrl(), d1_common.util.encodePathElement(status) + '/' +
  ...
  def replicate_store(self, pid, scidata):
    ...
    # Send REST POST call to register object. The URL is the same as for
    # /object/ GET.
    ...
    crud_create_url = urlparse.urljoin(
      self.getReplicationStoreUrl(), d1_common.util.encodePathElement(pid)
    )
