...
class ReplicationQueueProcessor(object):
  ...
  def process_replication_queue(self):
    queue_queryset = mn.models.ReplicationQueue.objects.filter(
  ...
  def _update_local_request_status(self, queue_model, status_str):
    mn.models.update_replica_status(queue_model.local_replica.info, status_str)
  ...
  def _remove_completed_requests_from_queue(self):
    mn.models.ReplicationQueue.objects.filter(
  ...
  def _create_replica(self, sysmeta_obj, sciobj_stream):
    ...
    sciobj_model = mn.sysmeta.create(sysmeta_obj, url)
  ...
  def _create_replica_obsolescence_reference(self, pid):
    mn.models.replica_obsolescence_chain_reference(pid)
  ...
  def _assert_is_pid_of_local_unprocessed_replica(self, pid):
    if not mn.sysmeta_replica.is_unprocessed_local_replica(pid):
      raise ReplicateError(
  ...
  def _assert_pid_is_unknown_or_replica(self, pid):
    if mn.sysmeta.is_did(pid) and not mn.sysmeta_replica.is_local_replica(pid):
      raise ReplicateError(
        u'The identifier is already in use on the local Member Node. '
        u'pid="{}"'.format(pid)
      )
