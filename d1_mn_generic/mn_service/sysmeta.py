...
#System metadata fields that must be filled by the client:
#
#* identifier for prototype, use "throwaway" GUID for zip and sci-meta
#
#* objectFormat Mime/Type (enumerated value from sysmetadata schema)
#
#* size byte size of object (science metadata or data zip file)
#
...
#* derivedFrom null
#
#* describes ID of the data (for system metadata describing science metadata)
#
#* describedBy ID of the science metadata (for sysmeta describing data)
#
...
def register_object_create_sysmeta(item, object_tree):
  ...
  date_sys_metadata_modified_el = etree.SubElement(
    system_metadata_el, 'DateSysMetadataModified'
  )
  ...
  authorative_member_node_el = etree.SubElement(
    system_metadata_el, 'AuthoritativeMemberNode'
  )
