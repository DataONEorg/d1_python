...
class Test_ResourceMapGenerator(TestCaseWithURLCompare):
...
#         self.parser = data_package.ResourceMapParser()
    ...
    # 
    #=========================================================================
    @patch.object(data_package.ResourceMapGenerator,'_serialize_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid')
    def test_simple_generate_resource_map(self, mock_aggregate, mock_generate, mock_serialize):
        ...
        response = self.generator.simple_generate_resource_map(resource_map_pid, science_metadata_pid,
                                     science_data_pids)
        self.assertEqual('test',response)
   
    @patch.object(data_package.ResourceMapGenerator,'_serialize_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    def test_simple_generate_resource_map_assert_called_aggregation_uri_from_pid(self, mock_generate, mock_serialize):
        with patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid') as mocked_method:
            resource_map_pid = '_bogus_pid_459837453495884543459873'
            science_metadata_pid = '_bogus_pid_845434598734598374534958'
            science_data_pids = '_bogus_pid_983745349845434598734558'
            self.generator.simple_generate_resource_map(resource_map_pid, science_metadata_pid,
                                         science_data_pids)
            mocked_method.assert_called_with(resource_map_pid)         
    
    @patch.object(data_package.ResourceMapGenerator,'_serialize_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid')
    def test_simple_generate_resource_map_assert_called_generate_resource_map(self, mock_aggregate, mock_serialize):
        with patch.object(data_package.ResourceMapGenerator,'_generate_resource_map') as mocked_method:
            resource_map_pid = '_bogus_pid_459837453495884543459873'
            science_metadata_pid = '_bogus_pid_845434598734598374534958'
            science_data_pids = '_bogus_pid_983745349845434598734558'
    ...
    @patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid')
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    def test_simple_generate_resource_map_assert_called_serialize_resource_map(self, mock_generate, mock_aggregate):
        with patch.object(data_package.ResourceMapGenerator,'_serialize_resource_map') as mocked_method:
            resource_map_pid = '_bogus_pid_459837453495884543459873'
            science_metadata_pid = '_bogus_pid_845434598734598374534958'
            science_data_pids = '_bogus_pid_983745349845434598734558'
    ...
    @patch.object(data_package.ResourceMapGenerator,'_generate_sys_meta')
    @patch.object(d1_common.checksum,'create_checksum_object')
    @patch.object(data_package.ResourceMapGenerator,'_serialize_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid')
    def test_generate_system_metadata_for_resource_map(self, mock_aggregate, mock_generate, mock_serialize, mock_checksum,mock_sysmeta):
        resource_map_pid = '_bogus_pid_459837453495884543459873'
        mock_checksum.return_value = 'test1'
        mock_sysmeta.return_value = 'sysmeta'
        response = self.generator.generate_system_metadata_for_resource_map(resource_map_pid, 'test','user')
    ...
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid')
    def test_generate_system_metadata_for_resource_map_assert_called_generate_sys_meta(self, mock_aggregate, mock_generate, mock_serialize, mock_checksum):
        with patch.object(data_package.ResourceMapGenerator,'_generate_sys_meta') as mocked_method:
            resource_map_pid = '_bogus_pid_459837453495884543459873'
            mock_generate.return_value = 'test'
            mock_checksum.return_value = 'test1'
            self.generator.generate_system_metadata_for_resource_map(resource_map_pid, 'test','user')
            mocked_method.assert_called_with('_bogus_pid_459837453495884543459873', 'http://www.openarchives.org/ore/terms', 4, 'test1', 'user')      
    
    @patch.object(data_package.ResourceMapGenerator,'_generate_sys_meta')
    @patch.object(data_package.ResourceMapGenerator,'_serialize_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    @patch.object(data_package.ResourceMapGenerator,'_aggregation_uri_from_pid')
    def test_generate_system_metadata_for_resource_map_assert_called_create_checksum_object(self, mock_aggregate, mock_generate, mock_serialize, mock_checksum):
        with patch.object(d1_common.checksum, 'create_checksum_object') as mocked_method:
            resource_map_pid = '_bogus_pid_459837453495884543459873'
            mock_generate.return_value = 'test'
            mock_checksum.return_value = 'test1'
            self.generator.generate_system_metadata_for_resource_map(resource_map_pid, 'test','user')
            mocked_method.assert_called_with('test',d1_common.const.DEFAULT_CHECKSUM_ALGORITHM)      
    
    @patch('data_package.foresite.ResourceMap')
    @patch('data_package.foresite.AggregatedResource') 
    @patch('data_package.foresite.utils') 
    @patch('data_package.foresite.Aggregation')        
    @patch.object(rdflib.namespace,'Namespace')
    @patch('data_package.foresite.utils.Namespace')
    @patch.object(data_package.ResourceMapGenerator,'_resolvable_uri_from_pid')
    def test_generate_resource_map(self, mock_resolve, mock_foresite, mock_namespace, mock_aggregation, mock_utils,mock_resource,mock_map):
        ...
        response = self.generator._generate_resource_map('aggregation_id', 'resource_map_id', relations)
    ...
    @patch.object(rdflib.namespace,'Namespace')
    @patch('data_package.foresite.utils.Namespace')
    def test_generate_resource_map_assert_called_resolvable_uri_from_pid(self, mock_foresite, mock_namespace, mock_aggregation, mock_utils,mock_resource,mock_map):
        with patch.object(data_package.ResourceMapGenerator, '_resolvable_uri_from_pid') as mocked_method:
            relations = {'SCIMETA_PID': ['SCIDATA_PID_1', 'SCIDATA_PID_2']}
            mock_namespace.return_value = 'test'
            mock_aggregation.return_value = foresite()
            mock_resource.return_value = foresite()
            mock_map.return_value = foresite()
            self.generator._generate_resource_map('aggregation_id', 'resource_map_id', relations)
    ...
    @patch.object(rdflib.namespace,'Namespace')
    @patch('data_package.foresite.utils.Namespace')
    def test_generate_resource_map_assert_number_calls_resolvable_uri_from_pid(self, mock_foresite, mock_namespace, mock_aggregation, mock_utils,mock_resource,mock_map):
        with patch.object(data_package.ResourceMapGenerator, '_resolvable_uri_from_pid') as mocked_method:
            relations = {'SCIMETA_PID': ['SCIDATA_PID_1', 'SCIDATA_PID_2']}
            mock_namespace.return_value = 'test'
            mock_aggregation.return_value = foresite()
    ...
    @patch('data_package.foresite.ResourceMap')
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    @patch('data_package.foresite.RdfLibSerializer')
    @patch.object(data_package.ResourceMapGenerator,'_assert_is_valid_serialization_format')
    def test_serialize_resource_map(self, mock_assert, mock_foresite, mock_generate,mock_map):
    ...
    @patch.object(data_package.ResourceMapGenerator,'_generate_resource_map')
    @patch('data_package.foresite.RdfLibSerializer')
    def test_serialize_resource_map_assert_called_assert_is_valid_serialization_format(self, mock_foresite, mock_generate,mock_map):
        with patch.object(data_package.ResourceMapGenerator, '_assert_is_valid_serialization_format') as mocked_method:
            mock_foresite.return_value = foresite()
            mock_generate.return_value = foresite()
            mock_map.get_serialization().data.return_value = 'data'
    ...
    def test_resolvable_uri_from_pid_assert_called_URIRef(self):  
        with patch.object(rdflib.term,'URIRef') as mocked_method:
            self.generator._resolvable_uri_from_pid('_bogus_pid_459837453495884543459873')
    ...
    def test_aggregation_uri_from_pid_assert_called_URIRef(self):  
        with patch.object(rdflib.term,'URIRef') as mocked_method:
            self.generator._aggregation_uri_from_pid('_bogus_pid_459837453495884543459873')
...
#         mock_time.return_value = '2015-03-06 12:56:54.323738'
        with patch('data_package.datetime.datetime') as mock_datetime:
            ...
            output = self.generator._generate_sys_meta('_bogus_pid_459837453495884543459873', 'http://www.openarchives.org/ore/terms', 10, 'checksum', 'rights_holder')
    ...
    def test_generate_public_access_policy(self,mock_policy,mock_rule):
        ...
        self.assertEqual([u'read'], output.__dict__['return_vals'][0].__dict__['permission'].return_vals)
        self.assertEqual( ['public'], output.__dict__['return_vals'][0].__dict__['subject'].return_vals)
#=========================================================================
class Test_ResourceMapParser(TestCaseWithURLCompare):
...
#         self.parser = data_package.ResourceMapParser()
    ...
    @patch.object(data_package.ResourceMapParser,'_parse')
    ...
    @patch('data_package.foresite.RdfLibParser.parse')
    @patch('data_package.foresite.RdfLibParser')
    @patch('data_package.foresite.ReMDocument')
    ...
    @patch('data_package.foresite.ResourceMap.graph')    
    @patch.object(data_package.ResourceMapParser,'get_resource_map')
