...
class D1HttpClient_httplib2(httplib2.Http):
  '''Implements a D1HttpClient using the httplib2 client.
  '''

  def __init__(self, args, kwargs):
    httplib2.HttpClient.__init__(self, *args, **kwargs)

  def request(self, *args, **kwargs):
    '''Overrides httplib2 Http.request.
    ...
    '''
    try:
      self._last_response, data = super(D1HttpClient_httplib2, self).request(
        *args, **kwargs
      )
...
class D1client(object):
  '''Implements a basic client for interaction with the DataONE system.  The
  actual work is implemented through a HttpClient instance and the responses.
  '''

  def __init__(
    self,
    introspectionTarget=d1const.URL_DATAONE_ROOT,
    ...
    '''
    :param introspectionTarget: Location of a service for discovering
      information about the DataONE system, such as the locations of resolvers, 
      coordinating nodes and member nodes.  For normal operations this will be
      cn.dataone.org.  Alternative targets may be specified for testing and
      development purposes.
    :type introspectionTarget: string  
    :param httpParams: Dictionary of parameters that should be passed on to the 
      http client.
    :type httpParams: dictionary
    :param httpClientClass: Name of the HTTP client class to use
    :type httpClientClass: string
    
    :param cache: Optional, either the name of a directory to be used as a flat
      file cache, or an object that implements httlib2.FileCache interface
    :param timeout: Optional socket level connection timeout (seconds)
    :param proxy_info: Optional httplib2.ProxyInfo instance
    '''
    self.logger = logging.getLogger(self.__class__.__name__)
    self.d1root = introspectionTarget
    self.httpParams = httpParams
    self._httpClient = None
    self.httpClientClass = "urllib2.urlopen"
  ...
  def _getHttpClient(self):
    '''Internal factory method that returns an object implementing the 
    HttpClient interface.
    '''
    if self._httpClientClass == 'urllib2.urlopen':
      self._httpClient = None

    if self._httpClientClass == 'D1HttpClient_httplib2':
  ...
  #uri, method="GET", body=None, headers=None, redirections=DEFAULT_MAX_REDIRECTS, connection_type=None  
  #  def GET(self, url, headers=None, 
  #          redirections=DEFAULT_MAX_REDIRECTS, connection_type=None):
  #    client = self._getHttpClient()
  #    self._last_response = client
  ...
  def resolve(self, guid):
    ...
    url = self._normalizeTarget(self.d1root)
    url = u"%s/%s/locate" % (url, guid)
    headers = self._getHeaders()
    response, data = self.request(url, method='GET', headers=headers)
  ...
  def get(self, guid, target=None):
    ...
    :TODO: Since the response may be quite large for real data sets, need to
      modify the response to return the stream rather than the content read
      from the stream.
  ...
  def updateSystemMetadata(self, sysmeta, target=None):
    '''Updates the system metadata document held on target using the HTTP PUT
    method.  The sysmetadata guid 
    
    :param sysmeta: An instance of D1SystemMetadata
    :param target: The location to receive the update version of system metadata
    :rtype:  
    '''
    raise NotImplementedError('updateSystemMetadata method is not implemented.')
