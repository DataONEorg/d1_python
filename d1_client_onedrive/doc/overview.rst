:doc:`/onedrive/index` enables DataONE objects stored in `Zotero citation manager`_ libraries to be accessed like regular files on Windows, Mac OS X and Linux systems. This allows users to open remote DataONE objects locally and work with them as if they reside on the user's computer. For instance, a spread sheet that is stored on a Member Node can be opened directly in Excel.
