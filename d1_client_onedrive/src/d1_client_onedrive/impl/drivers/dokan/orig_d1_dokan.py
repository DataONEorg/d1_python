  ...
  def getFileInformation(self, fileName):
    ...
    # related items
      ...
      # abstract.txt
        ...
        return dict(
          attr=attrs, ctime=now, atime=now, wtime=now, size=len(abstxt),
          nlinks=1
        )
    ...
    if len(tokens) == 5:
      ...
      if tokens[3] == 'describes':
        try:
          ...
          mtime1 = time.mktime(sysm.dateSysMetadataModified.timetuple())
          return dict(
            attr=attrs, ctime=ctime1, atime=now, wtime=mtime1, size=sysm.size,
            nlink=1
          )
        except:
  ...
  def findFilesWithPattern(self, fileName, searchPattern):
    ...
    if (len(pathTokens) == 1):
      ...
      if pathTokens[0].lower() in self.facets.keys():
        ...
        for v in facet_values:
          result.append(
            dict(
              name=v, attr=(FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_READONLY),
              ctime=now, atime=now, wtime=now, size=0
            )
          )
        return result
    ...
    # identifier level - contains data packages
      ...
      for entry in entries:
        result.append(
          dict(
            name=self.encodePathName(entry), attr=(
              FILE_ATTRIBUTE_DIRECTORY | FILE_ATTRIBUTE_READONLY
            ), ctime=now, atime=now, wtime=now, size=0
          )
        )
      ...
      result.append(
        dict(
          name='systemmetadata.xml', attr=(
            FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_READONLY
          ), ctime=ctime1, atime=now, wtime=mtime1, size=len(xml)
        )
      )
      # science metadata
      result.append(
        dict(
          name=self.encodePathName(fname), attr=(
            FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_READONLY
          ), ctime=now, atime=now, wtime=now, size=sysm.size
        )
      )
      return result
        ...
        for rel in relations['describes']:
          ...
          result.append(
            dict(
              name=self.encodePathName(fname), attr=(
                FILE_ATTRIBUTE_NORMAL | FILE_ATTRIBUTE_READONLY
              ), ctime=ctime1, atime=now, wtime=mtime1, size=sysm.size
            )
          )
        return result
...
def main():
  ...
  parser.add_option(
    '-b', '--baseurl', dest='baseurl', default=d1_common.const.URL_DATAONE_ROOT,
  ...
  parser.add_option(
    '-s', '--stderr', dest='stderr', action='store_true', default=False,
    help='Display messages to stderr'
  )
  parser.add_option(
    '-t', '--threads', dest='threads', default=5, type='int',
    help='Number of threads to use'
  )
  parser.add_option(
    '-u', '--unmount', dest='unmount', action='store_true', default=False,
    help='Unmount the specified drive'
  )
  ...
  d1fs = dokan.Dokan(
    DataONEFS(baseurl=opts.baseurl, filter_query=opts.filter), mountpoint,
    DriverOption, 0x19831116L, opts.threads
  )
