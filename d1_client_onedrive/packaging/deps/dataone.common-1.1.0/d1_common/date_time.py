...
'''
Module d1_common.date_time
==========================

:Synopsis: Utilities for handling date-times in DataONE.
:Created: 2010-08-07
:Author: DataONE (Dahl)
'''
...
def normalize_datetime_to_utc(date_time, timezone=None):
  ...
  if not tz_dt_present and not tz_arg_present:
    raise TypeError('Timezone information must be provided either within '
  ...
  if tz_dt_present and tz_arg_present and date_time.utcoffset() != \
    datetime.timedelta(minutes=timezone):
      raise TypeError('Timezone information was provided both within datetime '
