...
def get_sid(sysmeta_pyxb):
  return gmn.app.util.get_value(sysmeta_pyxb, 'seriesId')
...
def create_chain(sid, pid):
  ...
  - {pid} must exist and be verified to be a PID.
    gmn.app.views.asserts.is_pid()
  """
  sid_model = gmn.app.models.did(sid) if sid else None
  pid_model = gmn.app.models.did(pid)
  chain_model = gmn.app.models.ChainIdToSeriesID(
    sid=sid_model, head_pid=pid_model
  )
  chain_model.save()
  pid_to_chain_model = gmn.app.models.PersistentIdToChainID(
...
def add_pid_to_chain(sid, old_pid, new_pid):
  ...
  - Both {old_pid} and {new_pid} must exist and be verified to be PIDs
    gmn.app.views.asserts.is_pid()
  """
  old_pid_to_chain_model = gmn.app.models.PersistentIdToChainID.objects.get(
    pid__did=old_pid
  )
  new_pid_to_chain_model = gmn.app.models.PersistentIdToChainID(
    chain=old_pid_to_chain_model.chain,
    pid=gmn.app.models.IdNamespace.objects.get(did=new_pid)
  )
  new_pid_to_chain_model.save()

  if sid:
    chain_model = gmn.app.models.ChainIdToSeriesID.objects.get(sid__did=sid)
    assert chain_model.sid is None or chain_model.sid.did == sid, \
      'Cannot change SID for existing chain'
    chain_model.sid = gmn.app.models.did(sid)
    chain_model.save()
...
def update_sid_to_head_pid_map(pid):
  ...
  while sci_model.obsoleted_by is not None:
    sci_model = gmn.app.util.get_sci_model(sci_model.obsoleted_by.did)
  chain_model = gmn.app.models.PersistentIdToChainID.objects.get(
    pid__did=pid
  ).chain
...
def delete_chain(pid):
  """"""
  pid_to_chain_model = gmn.app.models.PersistentIdToChainID.objects.get(
    pid__did=pid
  )
  chain_model = pid_to_chain_model.chain
  pid_to_chain_model.delete()
  if not gmn.app.models.PersistentIdToChainID.objects.filter(
      chain=chain_model,
  ).exists():
    if chain_model.sid:
      # Cascades back to chain_model.
      gmn.app.models.IdNamespace.objects.filter(did=chain_model.sid.did).delete()
    else:
      chain_model.delete()
...
def _cut_head_from_chain(sciobj_model):
  new_head_model = gmn.app.util.get_sci_model(sciobj_model.obsoletes.did)
...
def _cut_tail_from_chain(sciobj_model):
  new_tail_model = gmn.app.util.get_sci_model(sciobj_model.obsoleted_by.did)
...
def _cut_embedded_from_chain(sciobj_model):
  prev_model = gmn.app.util.get_sci_model(sciobj_model.obsoletes.did)
  next_model = gmn.app.util.get_sci_model(sciobj_model.obsoleted_by.did)
...
def resolve_sid(sid):
  ...
  """
  return gmn.app.models.ChainIdToSeriesID.objects.get(sid__did=sid).head_pid.did
...
def get_sid_by_pid(pid):
  ...
  """
  sid_model = gmn.app.models.PersistentIdToChainID.objects.get(
    pid__did=pid
  ).chain.sid
  if sid_model:
...
#     sci_model = gmn.app.util.get_sci_model(sci_model.obsoletes.pid.did)
#   chain_pid_list = [sci_model.pid.did]
#   while sci_model.obsoleted_by:
#     sci_model = gmn.app.util.get_sci_model(
...
def is_sid(did):
  return gmn.app.models.ChainIdToSeriesID.objects.filter(sid__did=did).exists()
