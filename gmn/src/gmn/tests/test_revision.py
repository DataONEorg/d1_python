...
class TestCutFromChain(gmn.tests.gmn_test_case.GMNTestCase):
  def assert_cut_from_chain(self, client, pid, sid, pid_chain_list):
    with gmn.tests.gmn_mock.disable_auth():
      ...
      assert gmn.app.revision.is_in_revision_chain(sciobj_model)
      # Cut from the chain
      logging.debug(
      ...
      assert not gmn.app.revision.is_in_revision_chain(sciobj_model)
      # New chain is valid
  ...
  def test_0010(self):
    ...
    def test(client, sid=None):
      ...
      sciobj_model = gmn.app.util.get_sci_model(pid_chain_list[0])
      assert not gmn.app.revision.is_in_revision_chain(sciobj_model)
