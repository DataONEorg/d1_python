...
class TestCreateAndGetStandalone(gmn.tests.gmn_test_case.D1TestCase):
  @responses.activate
  ...
  def test_1010(self):
    ...
    def test(client, binding):
      pid, sid, send_sciobj_str, send_sysmeta_pyxb = self.create_obj(
        client, binding, now_dt=datetime.datetime(2010, 10, 10, 10, 10, 10)
      )

      with gmn.tests.gmn_mock.disable_auth_context():
        response = client.get(pid)

      self.assertEqual(
        response.headers['content-length'], str(len(send_sciobj_str))
      )
      self.assertTrue(
      ...
      self.assertEqual(
        response.headers['dataone-formatid'], 'application/octet-stream'
      )
      self.assertEqual(
        response.headers['last-modified'], 'Sun, 10 Oct 2010 10:10:10 GMT'
      )
  ...
  def test_1030(self):
    ...
    def test(client, binding):
      pid, sid, sent_sciobj_str, sysmeta_pyxb = self.create_obj(client, binding)
      recv_sciobj_str, recv_sysmeta_pyxb = self.get_obj(client, pid)
      self.assertEquals(sent_sciobj_str, recv_sciobj_str)
  ...
  def test_1070(self):
    ...
    def test(client, binding):
      pid, sid, sciobj_str, sysmeta_pyxb = self.create_obj(
        client, binding, permission_list=None
      )
      with self.assertRaises(d1_common.types.exceptions.NotAuthorized):
        self.get_obj(
          client,
  ...
  def test_1110(self):
    ...
    def test(client, binding):
      pid, sid, sciobj_str, sysmeta_pyxb = self.create_obj(
        client, binding, permission_list=[(['subj5'], ['changePermission'])]
      )
      self.get_obj(
        client,
        pid,
