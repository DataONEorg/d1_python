...
class Command(django.core.management.base.BaseCommand):
    ...
    async def _handle(self):
        aclient = d1_client.aio.async_client.AsyncDataONEClient(
        ...
        event_log_iterator = d1_gmn.app.management.commands.loginfo_async.EventLogIteratorAsync(
            aclient,
            page_size=self.options["page_size"],
            get_log_records_args_dict={
        ...
        async for log_pyxb in event_log_iterator.itr():
            i += 1
            self._logger.info(i)
            self._logger.info(log_pyxb.toxml())
