...
class ScienceObject(django.db.models.Model):
    ...
    format = django.db.models.ForeignKey(ScienceObjectFormat, django.db.models.CASCADE)
    filename = django.db.models.CharField(max_length=256, db_index=True, null=True)
...
def local_replica(pid, replica_info_model):
    local_replica_model = LocalReplica(
        pid=d1_gmn.app.did.get_or_create_did(pid), info=replica_info_model
    )
...
def replication_queue(local_replica_model, size):
    replication_queue_model = ReplicationQueue(
        local_replica=local_replica_model, size=size, failed_attempts=0
    )
...
class SystemMetadataRefreshQueue(django.db.models.Model):
    ...
    sciobj = django.db.models.OneToOneField(ScienceObject, django.db.models.CASCADE)
