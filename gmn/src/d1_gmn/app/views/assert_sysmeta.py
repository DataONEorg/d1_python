...
def sanity(request, sysmeta_pyxb):
    """Check that sysmeta_pyxb is suitable for creating a new object and
    matches the uploaded sciobj bytes."""
...
def has_matching_modified_timestamp(new_sysmeta_pyxb):
    ...
    if not d1_common.date_time.are_equal(old_ts, new_ts):
        raise d1_common.types.exceptions.InvalidRequest(
            ...
            'pid="{}" old_ts="{}" new_ts="{}"'.format(pid, old_ts, new_ts),
            identifier=pid,
        )
...
def _obsoleted_by_not_specified(sysmeta_pyxb):
    ...
    if obsoleted_by is not None:
        raise d1_common.types.exceptions.InvalidSystemMetadata(
            0,
            'obsoletedBy cannot be specified in System Metadata for this method.'
...
def obsoletes_not_specified(sysmeta_pyxb):
    ...
    if obsoletes_pid is not None:
        raise d1_common.types.exceptions.InvalidSystemMetadata(
            0,
            'obsoletes cannot be specified in System Metadata for this method. '
...
def obsoletes_matches_pid_if_specified(sysmeta_pyxb, old_pid):
    ...
    if obsoletes_pid is not None and obsoletes_pid != old_pid:
        raise d1_common.types.exceptions.InvalidSystemMetadata(
            0,
            'Persistent ID (PID) specified in System Metadata "obsoletes" '
...
def is_valid_sid_for_new_standalone(sysmeta_pyxb):
    """Assert that any SID in ``sysmeta_pyxb`` can be assigned to a new
    standalone object."""
...
def is_valid_sid_for_chain(pid, sid):
    """Assert that ``sid`` can be assigned to the single object ``pid`` or to
    the chain to which ``pid`` belongs.
...
def _does_not_contain_replica_sections(sysmeta_pyxb):
    """Assert that ``sysmeta_pyxb`` does not contain any replica
    information."""
...
def _is_not_archived(sysmeta_pyxb):
    """Assert that ``sysmeta_pyxb`` does not have have the archived flag
    set."""
...
def _has_correct_file_size(request, sysmeta_pyxb):
    if sysmeta_pyxb.size != request.FILES['object'].size:
        raise d1_common.types.exceptions.InvalidSystemMetadata(
            0,
            'Object size in System Metadata does not match that of the '
...
def _is_supported_checksum_algorithm(sysmeta_pyxb):
    if not (d1_common.checksum.is_supported_algorithm(sysmeta_pyxb.checksum.algorithm)):
        raise d1_common.types.exceptions.InvalidSystemMetadata(
            0,
