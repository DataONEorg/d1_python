...
class TestSysmetaUtil(d1_gmn.tests.gmn_test_case.GMNTestCase):
  ...
  def test_1000(self):
    ...
    def test(client):
      ...
      assert d1_gmn.app.models.Subject.objects.count() == 0
      ...
      qs = d1_gmn.app.models.Subject.objects.all().order_by('subject'
                                                            ).values('subject')
      ...
      d1_gmn.app.models.whitelist_for_create_update_delete('rights_holder_subj')
