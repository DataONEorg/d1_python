...
class D1FS:
    def __init__(self, baseurl=d1_common.const.URL_DATAONE_ROOT, filter_query=None):
        ...
        # Solr search facets.
        self.facets = {
            'projects': {'f': 'project', 'tstamp': 0, 'v': [], 'n': []},
            'data_providers': {'f': 'datasource', 'tstamp': 0, 'v': [], 'n': []},
            'decade': {'f': 'decade', 'tstamp': 0, 'v': [], 'n': []},
            'keywords': {'f': 'keywords', 'tstamp': 0, 'v': [], 'n': []},
            'title': {'f': 'title', 'tstamp': 0, 'v': [], 'n': []},
        }
    ...
    def getRecords(self, facet, term):
        ...
        records = solrclient.SOLRArrayResponseIterator(
            sc, q, fq=self._filter_query, cols=self.fields, pagesize=ITERATOR_PER_FETCH
        )
        return records
    ...
    def getAbstract(self, pid):
        ...
        records = solrclient.SOLRArrayResponseIterator(
            sc, q, fq=self._filter_query, cols=['abstract'], pagesize=ITERATOR_PER_FETCH
        )
        ret = ''
        for rec in records:
