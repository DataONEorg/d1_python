...
def redbaron_tree_to_module_str(baron_tree, strict=False):
    return UnicodeRenderWalker(strict=strict).dump(baron_tree.fst()).encode('utf-8')
...
def update_module_file_ast(ast_tree, module_path, show_diff=False, update=False):
    with tempfile.NamedTemporaryFile() as tmp_file:
...
def has_doc_str(node):
    return isinstance(node.value[0], redbaron.nodes.StringNode) or isinstance(
        node.value[0], redbaron.nodes.UnicodeStringNode
    )
