...
def main():
  ...
  with open(args.fout, 'w') as f:
    json.dump({
      'checksum_validation_list': checksum_validation_list
    }, f, indent=2)
...
def make_checksum_validation_script(stats_list):
  ...
  with open('./hash_check/curl.sh', 'w') as curl_f, \
    open('./hash_check/md5.txt', 'w') as md5_f, \
    open('./hash_check/sha1.txt', 'w') as sha1_f:
    ...
    for stats_dict in stats_list:
      for sysmeta_xml in stats_dict['largest_sysmeta_xml']:
        ...
        curl_f.write(
          ...
          format(file_name, base_url, d1_common.url.encodePathElement(pid))
        )
