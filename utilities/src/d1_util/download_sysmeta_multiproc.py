...
def main():
  ...
  api_major = (
    args.major if args.major is not None else
    d1_client.d1client.get_api_major_by_base_url(args.baseurl)
  )
  ...
  try:
    ...
    _download_objects(
      args, api_major, events, args.timeout, args.cert_pem_path,
      args.cert_key_path
    )
...
def parse_cmd_line():
  ...
  parser.add_argument(
    'dst_dir_path',
    help='Path to directory in which to store downloaded objects'
  )
  return parser.parse_args()


def _download_objects(
    args, api_major, events, timeout, cert_pem_path, cert_key_path
):
  logging.info(
    'Downloading SysMeta and Science Objects from Node. baseurl="{}"'.
    format(args.baseurl)
  )
    ...
    if d1_common.system_metadata.is_sysmeta_pyxb(sysmeta_pyxb):
      ...
      try:
        ...
        _download_source_sciobj_bytes(
          args.baseurl, api_major, pid, events, args.dst_dir_path, args.timeout,
          args.cert_pem_path, args.cert_key_path
        )
      except d1_common.types.exceptions.DataONEException as e:
        logging.error(str(e))
      else:
...
def _get_source_sysmeta(
    baseurl, api_major, pid, timeout, cert_pem_path, cert_key_path
):
...
def _download_source_sciobj_bytes(
    baseurl, api_major, pid, events, dst_dir_path, timeout, cert_pem_path,
    cert_key_path
):
...
def _get_safe_filename(pid, ext_str=None):
  return (
    urllib.parse.quote(pid.encode('utf-8'), safe=' @$,~*&') +
    '.{}'.format(ext_str) if ext_str is not None else ''
  )
