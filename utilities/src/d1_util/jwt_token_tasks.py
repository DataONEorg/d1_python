...
def validate_and_decode(jwt_bu64, cert_obj):
  ...
  library. Note that this does NOT validate the claims in the claim set."""
...
def find_valid_combinations(cert_file_name_list, jwt_file_name_list):
  ...
  combination along with indicators for combinations where the JWT signature was
  ...
  for cert_file_name in cert_file_name_list:
    ...
    for jwt_file_name in jwt_file_name_list:
      ...
      logging.info(
        '{} {} {}'.
        format('***' if is_ok else '   ', cert_file_name, jwt_file_name)
      )
...
def cert_cleanup():
  ...
  for cert_path in cert_path_list:
    if not os.path.isfile(cert_path
                          ) or os.path.getsize(cert_path) > 10 * 1024**2:
      continue
