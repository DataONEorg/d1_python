...
- Given a {subj_info_pyxb} object and a {auth_subj_str}, return {authn_set}, a set of
authenticated subjects.

- {primary_str} is a DataONE subject, typically a DataONE compliant serialization of the
DN of the DataONE X.509 v3 certificate extension from which the SubjectInfo was
extracted.

- {auth_subj_str} can be viewed as the root of a tree. Any subject in the
{subj_info_pyxb} that is directly or indirectly connected to the root is included in the
{authn_set}.

- {authn_set} will always include {auth_subj_str}.

- All subjects in {authn_set} are equivalent to {auth_subj_str} for the purpose of
access control for private science objects.

- If {subj_info_pyxb} does not contain all relevant records, it is still considered to
be valid, but {authn_set} will be incomplete.

- Only the subject strings and relationships in {subj_info_pyxb} are used by this
...
- To determine if an action is authorized, {authn_set} is checked against the
...
- {authn_set} is generated from {subj_info_pyxb} and {auth_subj_str} using the following
procedure:
...
access level for a given subjects needs to be specified in the ACL.
...
DataONE supports a system where subjects can be linked into equivalent identities as
well as managed in groups. E.g., a group of subjects can be created and all the subjects
in the group can be given access to an object by only listing the single group subject
in the object's ACL.
...
access should be granted. E.g., if an ACL contains the subject for a group, that group
subject must be in the list of subjects that the ACL is checked against.
...
object regardless of if the ACL grants access to the authenticated subject directly, or
to the equivalent identity or the group.
...
group. However, including the subject that is in a group, in the ACL, does not give
access to the other subjects in the group or to the group itself. In other words, groups
...
transfer of accesss from groups to their subjects means that, when a subject is found to
...
- For the purpose of this function, the SubjectInfo contains a set of statements that
establish subject types and relationships between subjects. There are 4 kinds of
statements:
...
- An equivalent subject can only be the equivalent for a person
  - The equivalence relationship is the only one that causes each side to be granted all the rights of the other side, and so allows the two subjects to be used interchangeably.
  - The other relationships cause one side to be granted the rights of the other side, but not the other way around.
  - E.g.: Designating a subject as a member of a group causes the subject to be granted the rights of the group, but does not cause the group to be granted the rights of the subject

- An equivalent subject can be an equivalent for a person, not a group
  - However, an equivalent subject can be a member of a group
...
Handling of various invalid SubjectInfo and corner cases:
  ...
  - person.isMemberOf and group.hasMember should always form pairs referencing each
  other.
    ...
    - person.isMemberOf subject references a person or equivalent instead of a group
      - Only Group subjects are searched for isMemberOf references, so only the referenced
      Group subject is added to the list of authorized subjects
  ...
  - person.isMemberOf subject does not reference a known subject
    ...
    authenticated subject, the whole record, including the isMemberOf subject is simply
    ignored
  ...
  - See the unit tests for example SubjectInfo XML documents for each of these issues and
...
def gen_subject_info_tree(subject_info_pyxb, authn_subj, include_duplicates=False):
    ...
    {authn_subj} are included in the tree.
    """
...
class SubjectInfoNode:
    ...
    @property
    def node_gen(self):
        """All nodes for the tree rooted at this node"""
    ...
    def leaf_node_gen(self):
        """All leaf nodes for the tree rooted at this node"""
    ...
    def get_path_str(self, sep=os.path.sep, type_str=None):
        """Path from root to this node as a string
        """
        return sep.join(
            list(
                reversed(
    ...
    def get_leaf_node_path_list(self, sep=os.path.sep, type_str=None):
        """Paths for all leaf nodes for the tree rooted at this node, list of strings
        """
        return [v.get_path_str(sep, type_str) for v in self.leaf_node_gen]

    def get_path_list(self, type_str=None):
        """Get a list of the label_str of the nodes leading up to this node from the root
        """
        return list(
            reversed(
    ...
    def get_label_set(self, type_str=None):
        """Return a set of label_str for the tree rooted at this node
        - If {type_str} is set, only return label_str extracted from nodes of that type
        are returned.
        """
        return {v.label_str for v in self.node_gen if type_str in (None, v.type_str)}

    def get_subject_set(self):
        """Return a set of subjects for the tree rooted at this node
        """
