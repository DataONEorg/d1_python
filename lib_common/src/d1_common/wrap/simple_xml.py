...
- Process XML that is invalid in such a day that PyXB cannot parse or generate
it.

- Process XML without causing xs:dateTime fields from being normalized to the
UTC time zone (PyXB is based on the XML DOM, which requires such normalization).
...
class SimpleXMLWrapper(object):
  ...
  def get_element(self, el_name, el_idx=0):
  ...
  def get_element_text(self, el_name, el_idx=0):
    ...
    return self.get_element(el_name, el_idx).text

  def set_element_text(self, el_name, el_text, el_idx=0):
    """Set the text of the selected element"""
    self.get_element(el_name, el_idx).text = el_text
  ...
  def remove_children(self, el_name, el_idx=0):
    ...
    self.get_element(el_name, el_idx)[:] = []
  ...
  def replace_by_etree(self, root_el, el_idx=0):
    ...
    """
    el = self.get_element(root_el.tag, el_idx)
    el[:] = list(root_el)
