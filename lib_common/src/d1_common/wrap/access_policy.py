...
class AccessPolicyWrapper(object):
  ...
  def get_normalized_perm_list(self):
    """
    Returns:
      A minimal, ordered, hashable list of subjects and permissions that represents the
  ...
  def clear(self):
    ...
    Only the rightsHolder set in the SystemMetadata will be able to access the object
    unless new perms are added after calling this method."""
    self._perm_dict = {}

  def add_public_read(self):
    """Add public public ``read`` perm.

    Add an allow rule with a ``read`` permission for the symbolic subject, ``public``.
    It is a no no-op if any of the existing rules already provide ``read`` or higher to
    ``public``.
    """
