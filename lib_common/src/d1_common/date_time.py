...
def cast_naive_datetime_to_tz(dt, tz=UTC()):
  """If datetime is tz-naive, set it to ``tz``.
  If datetime is tz-aware, return it unmodified.
...
def utc_now():
  """
  Returns:
     tz-aware datetime: The current local date and time adjusted to the UTC timezone.
