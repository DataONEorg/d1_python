...
def path_generator(
  ...
  Args:
    ...
    exclude_glob_list: list of str
      ...
      Patterns ending with "/" are matched only against dir
      names. All other patterns are matched only against file names. If the include list
      contains any file patterns, files must match one or more of the patterns in order
      to be returned. If the include list contains any dir patterns, dirs must match one
      or more of the patterns in order for the recursive search to descend into them.
    ...
    recursive: bool
      ...
      - **False**: Prevent searching subdirectories


    ignore_invalid: bool

      - **True**: Invalid paths in path_list are ignored.
      - False (default): EnvironmentError is raised if any of the paths in ``path_list`` do not
    ...
    return_dir_paths: bool
      ...
      - **True**: Directories paths are also returned.
